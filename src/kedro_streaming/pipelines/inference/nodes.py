...
def apply_json_schema(string_df):
    ...
    json_df = string_df.withColumn(
        "jsonData", F.from_json(F.col("value"), schema)
    ).select("jsondata.*")
    return json_df
...
def streaming_predict(stream_df, mlflow_model):
    def _pred(data):
        result = mlflow_model.predict(data).tolist()
        return result

    pred_udf = F.udf(_pred, ArrayType(IntegerType()))
    return stream_df.withColumn("predictions", pred_udf(F.col("data")))
